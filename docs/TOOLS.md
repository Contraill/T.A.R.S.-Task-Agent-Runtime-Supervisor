...
MCP tools remain externally supplied capabilities. Server-qualified names cannot shadow native tools. Connections and calls pass through the same policy, approval, audit and evidence path as native tools; unknown effects default to elevated denial. Trusted registry contracts derive call effects and targets from the validated argument snapshot actually sent to the server. A parallel caller-supplied target is not accepted. Spawned stdio helpers and detached descendants are parent-death supervised and quiesced on close; persistent external services use streamable HTTP. Streamable HTTP destinations and redirects receive SSRF validation. Registry configuration stores secret references rather than resolved values.
