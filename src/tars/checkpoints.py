...
def create_checkpoint_in_transaction(conn, task_id, state=None, *, reason="",
    ...
    for key, actual in (("task_id", task_id), ("owner_role", task["owner_role"]),
        ...
        if key in state and state[key] != actual:
            raise RuntimeError(f"checkpoint {key} does not match locked task snapshot")
    payload = json_dumps(state)
    digest = hashlib.sha256(payload.encode("utf-8")).hexdigest()
    cp_id = checkpoint_id or "cp-" + uuid.uuid4().hex
    ...
    conn.execute(
        ...
        (cp_id, task_id, task["epoch"], seq, stamp, task["owner_role"], reason,
         payload, json_dumps(list(evidence_refs) if evidence_refs is not None else
                             json_loads(task["evidence_refs_json"], [])), digest),
    )
    if advance_epoch:
        conn.execute("UPDATE tasks SET epoch=epoch+1,updated_at=? WHERE id=?",
                     (stamp, task_id))
    return conn.execute("SELECT * FROM checkpoints WHERE id=?", (cp_id,)).fetchone()
...
def create_checkpoint(task_id, state: dict | None = None, *, reason="", evidence_refs=None,
    ...
    with transaction(immediate=True) as conn:
        row = create_checkpoint_in_transaction(
            conn, task_id, state, reason=reason, evidence_refs=evidence_refs,
